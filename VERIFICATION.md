...
Revision: `reference-rebuild-2`

- 主模型重建：橫向屋脊、左側錯層、通往前廊的外梯、完整布面陽傘、拱形扇窗、沿屋簷生長的櫻花、立體磚牆與輪式底盤。
- 補齊後側磚牆與屋頂端面；修正無效材質索引造成的磚面缺失、反向瓦片、枝幹遮擋與錯誤傘面。
- 使用 GTAO、MSAA、微量 bloom 與連續攝影棚地面；靜止時按需渲染，避免持續消耗 GPU。
- 單元測試 8/8 通過；瀏覽器互動測試 6/6 通過。
- 正式 build 已通過；`scripts/verify-live.mjs` 已驗證桌機、平板、手機四種尺寸的 revision、WebGL、HTTP 及 console。
- 部分跨視角狀態測試使用 reduced motion，以免軟體 WebGL 被過場動畫拖慢；一般過場仍由獨立手機情境驗證。
- 分享縮圖為真實 3D 渲染。未展示面是合理補建；此模型仍屬程序化微縮模型，不能宣稱與原圖逐像素一致或已達照片級無瑕疵。
- 公開站點與來源庫沿用既有 GitHub Pages；線上驗收 JSON 存在 `test-results/live/verification.json`（不提交測試產物）。
